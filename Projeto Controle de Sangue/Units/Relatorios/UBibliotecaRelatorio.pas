...
type
  ...
  public
    ...
    class function ValidaFiltro(const pTIPOFILTRO: TTipoFiltro; const pLISTBOX: TListBox; const pEDIT: TEdit;
...
class function TBibliotecaRelatorio.ValidaFiltro(const pTIPOFILTRO: TTipoFiltro; const pLISTBOX: TListBox;
  const pEDIT: TEdit; var pMensagem: string): Boolean;
begin
  ...
  if (not Result) and (pTIPOFILTRO in [tfEntre, tfSomente, tfOmitir]) then
  begin

    pMensagem := IfThen(pTIPOFILTRO = tfEntre, 'É necessário dois selecionar dois registros para o filtro',
      'É necessário selecionar mais de um registro para filtro');
